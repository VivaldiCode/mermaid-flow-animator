...
- Vermelho se só recebeu `error`, verde se só `success`; se receber ambos, duas pílulas aparecem lado a lado (sucesso à esquerda, erro no canto).
...
Existe também um **plugin oficial para o [Obsidian](https://obsidian.md/)** que renderiza diagramas animados inline em suas notas. Mesmo motor (parser, layout, partículas), mas sem React — bundle de ~105 KB com renderização DOM/SVG imperativa via `MarkdownRenderChild`.
...
Tudo configurado em [`plugin/`](plugin/) — pasta self-contained com `manifest.json`, `package.json`, GitHub Actions de release e cópia das utilities core. Veja [plugin/README.md](plugin/README.md) (em inglês) ou [plugin/README.pt-BR.md](plugin/README.pt-BR.md) para instalação manual e instruções de submissão à Community Plugins do Obsidian.
...
- **Documentação**: mais screenshots, GIFs, novas traduções
...
- [x] ~~Tradução do README para inglês~~ ← feito
