...
- [Publishing for public use](#publishing-for-public-use)
...
mermaid-flow flow.mmd -o demo.gif
```

The published package bundles the web app's built assets, so no clone needed.
...
The project is structured so each component can be distributed independently. Here's the status and the steps required for each:

| Component | Public location | Status | Action required |
|---|---|---|---|
| **Web app** | [mermaid-flow-animator.pages.dev](https://mermaid-flow-animator.pages.dev/) | ✅ live | none |
| **Obsidian plugin** | [VivaldiCode/mermaid-flow-animator-obsidian](https://github.com/VivaldiCode/mermaid-flow-animator-obsidian) | published, releases automated via tag | submit PR to [obsidianmd/obsidian-releases](https://github.com/obsidianmd/obsidian-releases) — see [plugin/README.md](plugin/README.md#submission-to-the-obsidian-community-plugins-listing) |
| **CLI** | npm: `mermaid-flow-animator-cli` | configured, **not yet published** | see "Publishing the CLI to npm" below |
| **Claude Code skill** | `.claude/skills/mermaid-flow/` in this repo | live, install via symlink | optional: package as a Claude plugin once Anthropic ships an official marketplace |

### Publishing the CLI to npm

The CLI is fully configured for `npm publish` — `mermaid-flow-animator-cli@0.1.0` will be self-contained (web app bundled inside `web-dist/` via `prepublishOnly`).

**One-time setup the maintainer must do:**

1. **Create an npm account** at [npmjs.com](https://www.npmjs.com/signup) and enable 2FA.
2. **Verify the package name is available** (or pick a different one):
   ```bash
   npm view mermaid-flow-animator-cli
   # If "404 Not Found" → name is free
   # Otherwise → edit cli/package.json and pick a different name
   ```
3. **Create an npm automation token** at [npmjs.com/settings/<user>/tokens](https://www.npmjs.com/settings/) (type: "Automation"). Copy it.
4. **Add the token as `NPM_TOKEN` secret** in this repo: GitHub → Settings → Secrets and variables → Actions → New repository secret → name `NPM_TOKEN`, value paste.

**To publish version 0.1.0 the first time** (manual, requires your 2FA):
...
npm install
npm run build
npm run copy-web-dist      # copies ../dist into web-dist/

npm login                  # authenticates locally with 2FA
npm publish --access public
```

After the first publish, **future releases are automated** via [`.github/workflows/cli-release.yml`](.github/workflows/cli-release.yml):
...
# Tag with the cli-v prefix and push — the workflow builds + publishes
git tag cli-v0.1.1
git push origin main --tags
```

The workflow checks that the tag matches `cli/package.json#version`, builds the web app, runs `prepublishOnly` (which bundles `web-dist/`), and publishes with `--provenance` so npm's UI shows the build came from this repo.
